...
lines = []
with open('Countries.txt', 'r') as fid:
    lines = fid.readlines()


for i,l in enumerate(lines):
    old_pic = 'Pictures/Maps/{:02}.png'.format(i+1)
    shutil.copyfile(old_pic, 'Pictures/{}.png'.format(l.split()[0]))
...
while lines[i].strip():
    ...
    for u in useful[:-2]:
        countries.append([u.strip().replace(' ','_'), continent.replace(' ','_')])
...
while lines[i].strip():
    ...
    str2 = '{}\t{} {}'.format(countries[i2 - 1][0], countries[i2 - 1][1], ' '.join(borders))
    print(str2)
    lines2.append(str2)
    
    i += 1

with open('Countries.txt', 'w') as fid:
    fid.write('\n'.join(lines2))
    '''